...
function formatRule(job: any) {
  ...
  if (job.adjustmentType === "percentage") {
    return `${direction}${job.adjustmentValue}%`;
  }
...
function formatFeatures(job: any) {
  ...
  if (job.minPriceFloor || job.maxPriceCeiling) badges.push("🛡️ Guards");
  return badges.length > 0 ? badges.join(" • ") : "Standard";
}

function formatFilters(filtersJson: string): string {
  try {
    const filters = JSON.parse(filtersJson);
    const parts: string[] = [];
    if (filters.collectionId) parts.push("Collection filter");
    if (filters.tags?.length) parts.push(`Tags: ${filters.tags.join(", ")}`);
    if (filters.titleKeyword) parts.push(`Title: "${filters.titleKeyword}"`);
